...
- [x] Default local notification threshold is 80% used, configurable per device (`ai-limit-notifier config threshold N`) and stored only in that device's local config — never on the server. The server applies no percentage gate of its own: any authenticated, schema-valid submission it receives always creates/updates a durable event, since crossing the threshold is what caused the device to send it at all. Unit-tested at the local-config layer (out-of-range values fall back to the default; 1/80/100 round-trip) and the server layer (`TestUsageAlwaysCreatesADurableEvent`, including a used_percent below the old hardcoded 80% default).
